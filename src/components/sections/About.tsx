...
const pillars: {
...
}[] = [
  {
    ...
    points: ["React + Node.js", "PostgreSQL, MongoDB, Redis", "REST + WebSockets"],
  },
  {
    ...
    points: ["IDF platform architecture", "Code quality standards", "AI workflow adoption"],
  },
  {
    icon: BrainCircuit as LucideIcon, label: "AI-Native", sub: "Engineering", color: "#a78bfa",
    description: "AI is the first tool, not an afterthought. Integrated into every phase of the stack.",
    points: ["Claude, GPT-4, Copilot daily", "Local LLM via Ollama", "MCP server integrations"],
  },
  {
    icon: ShieldCheck as LucideIcon, label: "Security", sub: "First Mindset", color: "#34d399",
    description: "Security baked in from design. Every input validated, every API surface hardened.",
    points: ["Zod input validation at boundaries", "CSP headers + XSS prevention", "Prompt injection awareness"],
  },
];

function StatBlock({ value, suffix = "", label, started, delay = 0 }: {
...
export function About() {
  ...
  return (
    <div id="about" className="slide-section" ref={ref} style={{ background: "rgba(6,6,14,0.52)" }}>
        ...
        <div className="grid grid-cols-[1fr_1.2fr] gap-12 lg:gap-20 items-center">
          ...
          <div>
            <motion.h2
              ...
              style={{ fontSize: "clamp(2.2rem, 5.5vw, 5.5rem)", letterSpacing: "-0.03em" }}
            >
              Engineering<br />
              <span className="text-gradient-indigo whitespace-nowrap">with intention.</span>
            </motion.h2>
            ...
            >
              ...
              build, and ship end-to-end — from design to production. I lead teams and
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    borderColor: isOpen ? `${p.color}55` : "var(--border-subtle)",
                    ...
                    {isOpen ? (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <X className="w-3 h-3 opacity-40" style={{ color: p.color }} />
                        </div>
                        ...
                        <ul className="mt-1 flex flex-col gap-1">
                          {p.points.map((pt) => (
                            <li key={pt} className="flex items-start gap-1.5 text-[10px]" style={{ color: "var(--text-muted)" }}>
                              <span className="mt-1 w-1 h-1 rounded-full shrink-0" style={{ background: p.color }} />
                              {pt}
                            </li>
                          ))}
                        </ul>
                      ...
                      >
                        ...
                        <p className="text-base font-bold text-slate-100 leading-snug">{p.label}</p>
                        <p className="text-sm" style={{ color: "var(--text-muted)" }}>{p.sub}</p>
                        <p className="mt-2 text-[10px] tracking-wide" style={{ color: `${p.color}80` }}>
                          tap to expand →
                        </p>
                      </motion.div>
                    )}
