...
// ─── TODO: Replace onSubmit stub with a real Server Action or API route.
// See .env.local.example for the required env vars (RESEND_API_KEY, etc.).
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage: "radial-gradient(ellipse 70% 60% at 0% 100%, rgba(99,102,241,0.09) 0%, transparent 60%)",
      }}
    >
      <div className="h-full flex flex-col justify-center px-8 sm:px-14 lg:px-20 max-w-[1400px] mx-auto w-full">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <p className="mt-6 text-base leading-relaxed max-w-sm" style={{ color: "var(--text-secondary)" }}>
            ...
            <div className="mt-8 space-y-4">
              {[
                ...
                { icon: LinkedInIcon, label: "LinkedIn", value: "daniel-v-03b663152", href: personal.linkedin },
          ...
          >
            {submitted ? (
              <div className="flex flex-col items-center justify-center gap-5 p-10 rounded-2xl border text-center"
                ...
                <p className="text-sm max-w-xs" style={{ color: "var(--text-secondary)" }}>
                  Backend integration pending — reach out directly at{" "}
                  <a href={`mailto:${personal.email}`} className="underline" style={{ color: "var(--accent-indigo)" }}>
                    {personal.email}
                  </a>
                </p>
                <button onClick={() => setSubmitted(false)} className="text-xs underline" style={{ color: "var(--text-muted)" }}>
                  Send another
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
                {/* Honeypot */}
                <input {...register("_hp")} type="text" tabIndex={-1} aria-hidden="true"
                  className="absolute opacity-0 pointer-events-none w-0 h-0" autoComplete="off" />

                <div className="grid sm:grid-cols-2 gap-4">
                  <Field label="Name" error={errors.name?.message}>
                    <input {...register("name")} type="text" placeholder="Daniel Ventura"
                ...
                <Field label="Subject" error={errors.subject?.message}>
                  <input {...register("subject")} type="text" placeholder="Senior Full-Stack opportunity"
                    className={inputCls(!!errors.subject)} />
                </Field>

                <Field label="Message" error={errors.message?.message}>
                  <textarea {...register("message")} rows={4} placeholder="Tell me about the role or project..."
                ...
                <p className="text-[11px] text-center" style={{ color: "var(--text-muted)" }}>
                  ⚠ Backend TODO — validates only. Email{" "}
                  <a href={`mailto:${personal.email}`} className="underline hover:text-slate-300" style={{ color: "var(--accent-indigo)" }}>
                    directly
                  </a>{" "}
                  in the meantime.
                </p>
