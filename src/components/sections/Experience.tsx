...
function roleLines(role: string): string[] {
  ...
  if (words.length >= 3) return [...words.slice(0, -2), words.slice(-2).join(" ")];
  return words;
}
    ...
    <div id="experience" className="slide-section" ref={ref} style={{ background: "rgba(5,5,12,0.50)" }}>
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden" aria-hidden>
        <span className="font-black tracking-tighter select-none"
          style={{ fontSize: "clamp(5rem, 22vw, 24rem)", color: "rgba(255,255,255,0.018)", letterSpacing: "-0.06em", whiteSpace: "nowrap" }}>
          EXPERIENCE
        </span>
      </div>
      ...
      <div className="relative z-10 h-full flex flex-col justify-between px-8 sm:px-14 lg:px-20 py-10 max-w-[1400px] mx-auto w-full">
        ...
        <div className="flex items-center justify-between">
          <motion.p initial={{ opacity: 0, y: -10 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            className="text-xs font-semibold tracking-[0.3em] uppercase" style={{ color: "var(--accent-cyan)" }}>
            Career
          </motion.p>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            {experience.map((e, i) => (
              <button key={e.id} onClick={() => go(i)} className="flex flex-col items-center gap-1.5 group text-center" aria-label={`View ${e.role}`}>
                <span className="text-[10px] font-bold tracking-wider uppercase transition-colors duration-200 hidden sm:block"
                  style={{ color: i === active ? "var(--text-primary)" : "var(--text-muted)" }}>
                  {e.company.split(" ")[0]}
                </span>
                <span className="text-[10px] font-semibold tracking-wider transition-colors duration-200 whitespace-nowrap"
        ...
        <div className="flex-1 flex flex-col justify-center">
          <AnimatePresence mode="wait" custom={dir}>
            <motion.div key={active} custom={dir}
              ...
              <p className="text-xs font-semibold tracking-[0.3em] uppercase mb-3" style={{ color: "var(--accent-indigo)" }}>
                {job.period}
              </p>

              {/* Title — last two words kept together */}
              <h2 className="font-black leading-[1.0] tracking-tight text-gradient-subtle"
                style={{ fontSize: "clamp(2rem, 7.5vw, 8rem)", letterSpacing: "-0.04em", maxWidth: "90%" }}>
                {roleLines(job.role).map((line, i) => (
                  <span key={i} className="block">
                    {line.includes("Lead") || line.includes("Developer") || line.includes("Engineer")
              ...
              <p className="mt-1 text-lg sm:text-xl font-semibold" style={{ color: "var(--accent-cyan)" }}>
                {job.company}
              </p>

              <p className="mt-5 text-sm sm:text-base leading-relaxed max-w-2xl" style={{ color: "var(--text-secondary)" }}>
                {job.description}
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {job.tech.map((t) => (
                  <motion.span
                    ...
                    style={{
                      background: hoveredTech === t ? "rgba(99,102,241,0.2)" : "rgba(99,102,241,0.08)",
                      color: hoveredTech === t ? "#a5b4fc" : "var(--text-secondary)",
                      border: `1px solid ${hoveredTech === t ? "rgba(99,102,241,0.5)" : "rgba(99,102,241,0.2)"}`,
                      boxShadow: hoveredTech === t ? "0 0 12px rgba(99,102,241,0.3)" : "none",
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
              onClick={() => go(Math.max(active - 1, 0))} disabled={active === 0}
              className="p-2 rounded-xl border transition-colors disabled:opacity-30"
              style={{ borderColor: "var(--border-subtle)", color: "var(--text-secondary)" }}>
              <ChevronLeft className="w-4 h-4" />
            </motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}
              onClick={() => go(Math.min(active + 1, experience.length - 1))} disabled={active === experience.length - 1}
              className="p-2 rounded-xl border transition-colors disabled:opacity-30"
              style={{ borderColor: "var(--border-subtle)", color: "var(--text-secondary)" }}>
